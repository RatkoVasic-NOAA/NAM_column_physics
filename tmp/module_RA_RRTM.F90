...
!
      real(kind=kind_phys),DIMENSION((ITE-ITS+1),LM) ::                     &
                            plyr, qlyr, rhly, qstl, vvl, clw, tvly,     &
                            qc2d, qi2d, qs2d, ni2d,                     &
...
!
      DO L=1,LM
        ...
        VVEL(1:(ITE-ITS+1),L)=OMGALF(ITS:ITE,J,L)*1000.d0   &
                   *PRSL(1:(ITE-ITS+1),L)*RTvR(1:(ITE-ITS+1))            !not used
...
!  ---  compute diagnostic cloud related quantities
        ...
        call diagcld1                                                   &
!  ---  inputs:
     &     ( plyr,plvl,gt,rhly,vvl,cv,cvt1,cvb1,                        &
     &       xlat,xlon,slmsk,                                           &
     &       (ITE-ITS+1), LM, LMP1,                                              &
!  ---  outputs:
     &       clouds,cldsa_v,mtopa,mbota                                 &
...
!!  ---  inputs:
           ( PRSI,PLYR,PLVL,PRSLK,GT,GQ,GR1,SLMSK,RHLY,                 &
             ...
             ICSDSW,ICSDLW,NTOZ,NTRAC,NFXR,                             &  ! Use NTRAC instead of NUM_WATER
...
!=========================================================
      ...
      RLWTOA(ITS:ITE,J)=TOPFLW(1:(ITE-ITS+1))%upfxc
      RSWTOA(ITS:ITE,J)=TOPFSW(1:(ITE-ITS+1))%upfxc
...
!
            ...
            NCFRST(ITS:ITE,J)=NCFRST(ITS:ITE,J) + 1
!-- Added a time-averaged convective cloud fraction calculation
            ...
            NCFRCV(ITS:ITE,J)=NCFRCV(ITS:ITE,J)+1
         ELSE
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!-----------------------------------------------------------------------
      SUBROUTINE RRTM_INIT(SFULL,SHALF,PPTOP,                           &
...
!-----------------------------------------------------------------------
      ...
      REAL,DIMENSION(LM),INTENT(IN) :: SFULL, SHALF
!
      INTEGER :: I,IHRST,J,N
