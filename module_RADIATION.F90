...
!------------------------
!
      INTEGER,INTENT(IN) :: LM,I_INDEX,J_INDEX,MYPE                     &
                           ,IHRST,ITIMESTEP,JULDAY,JULYR                &
                           ,NPHS,NRADL,NRADS,NUM_WATER
!
      INTEGER,INTENT(IN) :: JDAT(8)
!
      INTEGER,DIMENSION(ITS:ITE,JTS:JTE),INTENT(INOUT) :: NCFRCV,NCFRST
!
...
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE),INTENT(IN) :: CUPPT               &
                                                   ...
                                                   ,MXSNAL,STDH
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,LM),INTENT(IN) :: Q,T,CW,O3        &
                                                      ,F_ICE,F_RAIN     &
                                                      ,F_RIMEF,OMGALF
!
!-- Modify ALBEDO array if MODIS albedos are used in the RRTM
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE),INTENT(INOUT) :: ACFRCV,ACFRST    &
                                                      ,RLWIN,RLWTOA     &
                                                      ,RSWIN,RSWOUT     &
                                                      ,HBOT,HTOP        &
                                                      ...
                                                      ,RSWINC,RSWTOA
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,LM),INTENT(INOUT) :: RLWTT,RSWTT
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE),INTENT(OUT) :: CFRACH,CFRACL      &
                                                    ,CFRACM,CZMEAN      &
                                                    ,SIGT4
!
      REAL,DIMENSION(:,:,:),INTENT(INOUT)::QC,QI,QS,QR,QG,NI
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,1:LM),INTENT(OUT) :: CLDFRA
!
...
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE) :: GSW                            &
     &                                  ,TOT,TSFC,XLAND                 &
     &                                  ,GLW,SWDOWN,SWDOWNC,CZEN        &
     &                                  ,CUPPTR
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,1:LM+1) :: PHINT
!
      REAL,DIMENSION(ITS:ITE,JTS:JTE,1:LM) :: PI3D                      &
                                             ,THRATEN,THRATENLW         &
                                             ,THRATENSW                 &
                                             ,PRL,RHO,QV                &
                                             ,QCW,QCI,QSNOW,NCI         &
...
!
!.......................................................................
!$omp parallel do private (j,i,k,pdsl,plyr,ql)
!.......................................................................
      DO J=JTS,JTE
      DO I=ITS,ITE
!
...
!-- Do nothing, since cloud fractions is calculated in RRTM
        ...
        CASE DEFAULT
          ...
          CALL CAL_CLDFRA(CLDFRA,QC,QI,F_QC,F_QI,ITS,ITE,JTS,JTE,LM)
...
!
        ...
        CASE (GFDLLWSCHEME)
                 ...
                 ,THRATENSW=thratensw                               &
                 ,IDS=its,IDE=ite, JDS=jts,JDE=jte, KDS=1,KDE=lm+1  &
                 ,IMS=its,IME=ite, JMS=jts,JME=jte, KMS=1,KME=lm+1  &
                 ,ITS=ITS,ITE=ITE, JTS=JTS,JTE=JTE, KTS=1,KTE=lm    &
                                                                    )
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!-----------------------------------------------------------------------
   SUBROUTINE cal_cldfra(CLDFRA,QC,QI,F_QC,F_QI,its,ite,jts,jte,lm)
!---------------------------------------------------------------------
   IMPLICIT NONE
!---------------------------------------------------------------------
   INTEGER,  INTENT(IN   )   :: its,ite, jts,jte, lm

!
   REAL, DIMENSION( its:ite, jts:jte, 1:lm ), INTENT(OUT  ) ::       &
                                                             CLDFRA

   REAL, DIMENSION( its:ite, jts:jte, 1:lm ), INTENT(IN   ) ::       &
                                                                 QI, &
                                                                 QC
